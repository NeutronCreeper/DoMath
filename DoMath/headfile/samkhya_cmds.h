...
void GCD(int flag){
    ...
    std::cin>>a>>b;
    if(std::cin.good()){
        std::cout<<"Greatest common divisor of ("<<a<<','<<b<<") is "<<gcdl(a,b)<<";\n";
    }else{
...
void LCM(int flag){
    ...
    std::cin>>a>>b;
    if(std::cin.good()){
        std::cout<<"Least common multiple of ("<<a<<','<<b<<") is "<<lcm(a,b)<<";\n";
    }else{
        throw "Not legal integers!\n";
    };
};
...
void ispow(int flag){
    ...
    if(std::cin.good()){
        if(ifpow(a,b)){
            std::cout<<a<<'='<<pow(a,(float)1/(float)b)<<'^'<<b<<";\n";
        }else{
            std::cout<<a<<" is not the "<<b<<" power of an integer;\n";
        };
    }else{
        throw "Not legal integers!\n";
    };
